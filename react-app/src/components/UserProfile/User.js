...
function User() {
  ...
  const pastAuctions = userAuctions.filter(auction => new Date(auction.end_date) > new Date(currentDate))
  const currentAuctions = userAuctions.filter(auction => new Date(auction.end_date) < new Date(currentDate))
